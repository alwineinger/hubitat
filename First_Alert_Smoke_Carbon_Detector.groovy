...
preferences {
    ...
    input name: "enableDescLog", type: "bool", title: "Enable descriptionText logging?", defaultValue: true
}
...
def parse(String description) {
	...
	debugLog("'$description' parsed to ${results.inspect()}")
	return results
}
...
def zwaveEvent(hubitat.zwave.commands.securityv1.SecurityMessageEncapsulation cmd, results) {
	...
	state.sec = 1
	debugLog("encapsulated: ${encapsulatedCommand}")
...
def descLog(msg) {
    if (settings?.enableDescLog != false) { // default true
